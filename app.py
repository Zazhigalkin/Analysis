...
if uploaded_file:
    try:
        ...
        if invalid_dates > 0:
            ...
            df = df[df['flight_date'].notna()].copy()  # Используем copy() чтобы избежать предупреждений
        ...
        # Дни до вылета (минимум 1 день) - исправленная версия
        df['days_to_flight'] = (df['flight_date'].dt.date - today).dt.days
        df['days_to_flight'] = df['days_to_flight'].clip(lower=1)

        # Проверка на рейсы, которые уже вылетели (days_to_flight = 0 после clip станет 1)
        past_flights_mask = (df['flight_date'].dt.date < today)
        past_flights = df[past_flights_mask]
        if not past_flights.empty:
            st.warning(f"⚠️ Найдено {len(past_flights)} рейсов, которые уже вылетели. Они будут исключены.")
            df = df[~past_flights_mask].copy()
