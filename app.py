...
with st.expander("ℹ️ ИНСТРУКЦИЯ ПО ИСПОЛЬЗОВАНИЮ И ЛОГИКЕ АНАЛИЗА"):
    ...
    - **sold_total** = `Cap - Av seats`  ← учитывает жёсткие блоки
...
with st.expander("📁 ФОРМАТ ЗАГРУЖАЕМОГО EXCEL-ФАЙЛА"):
    ...
    | `LF` | Load Factor | `98,7%` или `0,987` |
    | `Av fare` | Средний тариф | `4 039` |
    | `Cap` | Вместимость самолёта | `227` |
...
if uploaded_file:
    try:
        ...
        def classify(row):
            ...
            # 1. Перепродажа для малых планов с огромными продажами
            ...
            # 3. Малый план - упрощаем: всё равно требует контроля
            if daily_needed < 3:
                # Но если совсем не продаём - это проблема даже для малого плана
                if sold_yesterday == 0 and daily_needed > 0:
                    return "🔴 Отстаём"
                return "🟢 По плану"
            
            # 4. Далёкие рейсы с малым планом - теперь оцениваем по факту продаж
            if days_to_flight > 30 and daily_needed < 4:
                if sold_yesterday > daily_needed:
                    return "🔵 Перепродажа"
                elif sold_yesterday == 0:
                    return "🔴 Отстаём"  # Даже для далёких рейсов отсутствие продаж - проблема
                else:
                    return "🟢 По плану"  # Вместо "далеко до рейса"
